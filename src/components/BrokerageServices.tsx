    ...
    <div className="relative my-6">
      ...
      >
        {brokerageServices.map((item) => (
          <div
            className="p-7 bg-white rounded flex items-center justify-between shadow-lg group hover:bg-dark-blue duration-700 cursor-pointer"
            onClick={handleClick}
            key={item.title}
            id={item.title}
          >
            <div className="flex gap-5 items-center">
              ...
              <h3 className="text-dark-blue group-hover:text-sky-600 duration-700 font-medium text-lg">
                {item.title}
              </h3>
            </div>
            <div className="bg-medium-blue/20 p-3 rounded-full group-hover:bg-sky-300 duration-700">
              <IoIosArrowForward className="text-dark-blue group-hover:text-sky-600 duration-700" />
            </div>
          </div>
        ))}
      </div>
      ...
      >
        ...
        >
          <div className="p-3 bg-light-blue/15 rounded-full group-hover:bg-medium-blue duration-700">
            <IoIosArrowRoundBack className="text-dark-blue size-6 group-hover:text-sky-600 duration-700" />
          </div>
